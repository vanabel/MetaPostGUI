...
cd /path/to/MetaPostGUI
git pull
docker compose build metapostgui
docker compose up -d
docker compose ps
docker compose logs --tail=80 metapostgui
```

这类前端功能更新（例如 PNG 下载按钮）会被打进 `web/dist`，因此仅重启旧容器不够；必须先拿到新代码并重新构建镜像。普通 `build` 会复用 TeX 安装层；只有确认 Docker 缓存异常、旧前端资源仍被复用时，才改用 `docker compose build --no-cache metapostgui`。若你用 Synology Container Manager 的 Project 部署，先在项目目录更新代码，然后在 Project 中执行重新构建/重新部署；环境变量和端口映射保持原值即可。
...
仓库已附带 `Dockerfile` 与 `docker-compose.yml`，优先使用上文的 Docker Compose 部署；如果自行编写 Compose，仍应**只映射前端或 Nginx 端口**，API 保持在容器内 `127.0.0.1`。
